...
// Launcher for the MemoryLens.Mcp .NET global tool.
//
// This package ships no server code. It exists so the server can be started with
// `npx -y memorylens-mcp` by clients and directories that only know how to run
...
const PACKAGE_ID = "MemoryLens.Mcp";
const COMMAND = "memorylens-mcp";
const { version } = require("../package.json");

function fail(message) {
  process.stderr.write(`memorylens-mcp: ${message}\n`);
  process.exit(1);
}
...
// `memorylens.mcp   1.5.1   memorylens-mcp`. Local lookup — no network.
function installedVersion() {
  try {
    ...
    for (const line of output.split("\n")) {
      const match = line.match(/^memorylens\.mcp\s+(\S+)/i);
      if (match) {
        return match[1];
      }
    }
...
function install(alreadyInstalled) {
  ...
  process.stderr.write(
    `memorylens-mcp: ${verb === "install" ? "installing" : "updating"} ${PACKAGE_ID} ${version}...\n`
  );
  execFileSync(
    "dotnet",
...
if (!hasDotnet()) {
  fail(
    "the .NET SDK was not found on PATH. MemoryLens requires the .NET 10 SDK — https://dotnet.microsoft.com/download"
  );
}
...
if (current !== version) {
  try {
    ...
    process.stderr.write(
      `memorylens-mcp: could not update to ${version}, continuing with installed ${current}\n`
    );
  }
}
